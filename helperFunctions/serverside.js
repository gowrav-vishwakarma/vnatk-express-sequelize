...
module.exports = {
    ...
    if (Array.isArray(obj)) {
      for (let index = 0; index < obj.length; index++) {
        ...
        if (
          ...
          if (element.as === false) {
            obj[index] = Models.sequelize.fn(
              element.fn,
              Models.sequelize.col(element.col)
            );
          } else {
            obj[index] = [
              Models.sequelize.fn(
  ...
  getHeadersAndDeRef: function (model, req, Models, skipIdInsert) {
    ...
    if (
      req.body.read &&
      req.body.read.modeloptions &&
      req.body.read.modeloptions.attributes
    )
      fields = req.body.read.modeloptions.attributes;
    ...
    if (
      ...
      req.body.read.modeloptions &&
      req.body.read.modeloptions.attributes &&
      !req.body.read.modeloptions.attributes.includes(
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      const assosIndex = associations.findIndex(
        (o) => o.foreignKey == fields_info[fld].fieldName
      );
      if (req.body.read && req.body.read.autoderef && assosIndex > -1) {
        ...
        field_headers.push({
          ...
          value:
            associations[assosIndex].name.singular +
            "." +
            (Models[associations[assosIndex].model].titlefield
              ? Models[associations[assosIndex].model].titlefield
              : "name"), //TODO get titlefield from model
          sortable: fields_info[fld].sortable
            ? fields_info[fld].sortable
            : true,
        });
        // TODO add in model include if not set
        if (!req.body.read.modeloptions.include)
          req.body.read.modeloptions.include = [];
        inArrayAsString = req.body.read.modeloptions.include.includes(
          associations[assosIndex].name.singular
        );
        inArrayAsObjectInclude = req.body.read.modeloptions.include.findIndex(
          (o) =>
            o.model == associations[assosIndex].name.singular ||
            (o.as && o.as == associations[assosIndex].name.singular)
        );
        if (!inArrayAsString && inArrayAsObjectInclude == -1) {
          req.body.read.modeloptions.include.push(
            associations[assosIndex].as
              ? associations[assosIndex].as
              : associations[assosIndex].name.singular
          );
        }
        ...
        field_headers.push({
          text: fields_info[fld].caption
            ? fields_info[fld].caption
            : fields_info[fld].fieldName,
  ...
  editRecord: async function (
    ...
    if (modelFound) {
      if (
        beforeExecute &&
        beforeExecute(modelFound, "vnatk_edit", req, res, next) === false
      )
        return;

      const updated = await model
        .update(item, { where: where_condition, individualHooks: true })
        .catch((error) => {
          throw error;
        });
    }
  ...
  injectAddAction: function (model, req, Models) {
    ...
    if (
      req.body.create &&
      req.body.create.modeloptions &&
      req.body.create.modeloptions.attributes
    )
      addFields = req.body.create.modeloptions.attributes;
    var addAction = {
      name: "vnatk_add",
      caption: "Add",
      type: "NoRecord",

      formschema: VNATKClientHelpers.generateFormSchemaFromModel(
        model,
        addFields,
        Models
      ),
  ...
  injectEditAction: function (model, req, Models) {
    ...
    if (
      req.body.update &&
      req.body.update.modeloptions &&
      req.body.update.modeloptions.attributes
    )
      editFields = req.body.update.modeloptions.attributes;
    var editAction = {
      name: "vnatk_edit",
      caption: "Edit",
      type: "single",
  ...
  async AutoImportItem(
    ...
    if (item.$vnatk_data_handle) {
      $vnatk_data_handle = item.$vnatk_data_handle;
      delete item.$vnatk_data_handle;
    }
    if (item.$vnatk_find_options) {
      $vnatk_find_options = item.$vnatk_find_options;
      delete item.$vnatk_find_options;
    }
    if (item.$vnatk_cache_records) {
      $vnatk_cache_records = item.$vnatk_cache_records;
      delete item.$vnatk_cache_records;
    }
    if (item.$vnatk_update_data) {
      $vnatk_update_data = item.$vnatk_update_data;
      delete item.$vnatk_update_data;
    }
    if (item.$vnatk_find_options) {
      $vnatk_find_options = item.$vnatk_find_options;
      delete item.$vnatk_find_options;
    ...
    if ($vnatk_find_options.modelscope !== undefined) {
      if ($vnatk_find_options.modelscope === false) model = model.unscoped();
      else model = model.scope($vnatk_find_options.modelscope);
    }
    ...
    for (let index = 0; index < itemBelongsTo.length; index++) {
      ...
      if (_.has(item_orig, item_belongsto_relation.as)) {
        itemBelongsTo[index]["item"] = await module.exports
          .AutoImportItem(
            Models[item_belongsto_relation.model],
            item_orig[item_belongsto_relation.as],
            Models,
            undefined,
            undefined,
            transaction
        ...
        item[item_belongsto_relation.foreignKey] =
          itemBelongsTo[index]["item"][model.autoIncrementAttribute];
        // when finding this record, do we want the belongsToid part of where condition ? below code
        if (
          relation == "BelongsToMany" ||
          ($vnatk_find_options.modeloptions &&
            $vnatk_find_options.modeloptions.where &&
            $vnatk_find_options.modeloptions.where[
              item_belongsto_relation.foreignKey
            ] === true)
        )
          AdditionalWhere[item_belongsto_relation.foreignKey] =
            item[item_belongsto_relation.foreignKey];
    ...
    if ($vnatk_find_options.modeloptions) {
      ...
      let t_sent_mo = {
        where: module.exports.senitizeModelOptions(
          $vnatk_find_options.modeloptions,
          model,
          Models
        ),
    ...
    if (!_.isEmpty(AdditionalWhere)) {
      ...
      senitizedmodeloptions.where = Object.assign(
        senitizedmodeloptions.where,
        AdditionalWhere
      );
      plainCondition.where = Object.assign(
    ...
    switch ($vnatk_data_handle.toLowerCase()) {
      case "alwayscreate":
        item = await model
          .create(item, { transaction, individualHooks: true })
          .catch((err) => {
            throw err;
          });
        break;
      case "findorcreate":
        senitizedmodeloptions.transaction = transaction;
        t = await model.findOne(senitizedmodeloptions).catch((err) => {
          throw err;
        });
        if (!t) {
          t = await model
      ...
      case "findandupdateall":
        ...
        t = await model
          .update($vnatk_update_data, senitizedmodeloptions)
          .catch((err) => {
            throw err;
          });
        break;
    ...
    for (let index = 0; index < itemHasMany.length; index++) {
      ...
      if (_.has(item_orig, thishasmanyrelation.as)) {
        for (let j = 0; j < item_orig[thishasmanyrelation.as].length; j++) {
          ...
          let id_merge = {};
          id_merge[thishasmanyrelation.foreignKey] =
    ...
    for (let index = 0; index < itemBelongsToMany.length; index++) {
      ...
      if (_.has(item_orig, thisbelongstomanyrelation.as)) {
        for (
          let j = 0;
          j < item_orig[thisbelongstomanyrelation.as].length;
          j++
        ) {
          let thisitemdetails = item_orig[thisbelongstomanyrelation.as][j];
          let id_merge = {};
          id_merge[thisbelongstomanyrelation.foreignKey] =
            item[model.autoIncrementAttribute];
          thisitemdetails = Object.assign(thisitemdetails, id_merge);
          let AddWhere = id_merge;
          // if (thisitemdetails.$vnatk_find_options && thisitemdetails.$vnatk_find_options.modeloptions && thisitemdetails.$vnatk_find_options.modeloptions[thisbelongstomanyrelation.foreignKey] === false) {
